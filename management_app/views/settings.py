...
def point_policy():
    ...
    rules = []
    for row in rows:
        rules.append({"rule_id": row.rule_id, "rule_name": row.rule_name, "value": row.value})
...
def get_users_and_admins():
    ...
    #     """)
    stmt = db.select(Users.user_id, Users.user_name, Users.user_email, Users.user_ucinetid, Users.admin, FacultyStatus.role).\
        select_from(Users).\
        outerjoin(FacultyStatus, Users.user_id == FacultyStatus.user_id).\
        where(
            (FacultyStatus.end_year.is_(None) & FacultyStatus.active_status.is_(True)) | 
            (FacultyStatus.role.is_(None))
        )
    ...
    for user in res:
        if user.admin:
            # admins.append(user)
            admins.append({"user_id": user.user_id, "user_name": user.user_name, "user_email": user.user_email, "user_ucinetid": user.user_ucinetid, "admin": user.admin, "role": user.role})
        else:
            # users.append(user)
            users.append({"user_id": user.user_id, "user_name": user.user_name, "user_email": user.user_email, "user_ucinetid": user.user_ucinetid, "admin": user.admin, "role": user.role})
...
def add_admin_status(id):
    ...
    # db.commit()
    stmt = db.update(Users).values(admin=1).where(Users.user_id == id)
...
def remove_admin_status(id):
    ...
    # db.commit()
    stmt = db.update(Users).values(admin=0).where(Users.user_id == id)
    db.session.execute(stmt)
    db.session.commit()
...
def delete_admin(id):
    ...
    # db.commit()
    db.session.execute(db.delete(Users).where(Users.user_id == id))
    db.session.commit()
...
def is_only_admin():
    ...
    res = db.session.execute(db.select(db.func.count(Users.admin)).where(Users.admin.is_(True))).scalar_one()
...
def update_rule_point_value(id, value):
    ...
    old = db.session.execute(db.select(Rules.value).where(Rules.rule_id == id)).scalar_one()

    # db.execute('UPDATE rules SET value = ? WHERE rule_id = ?', (value, id))
    # db.commit()
    db.session.execute(db.update(Rules).values(value=value).where(Rules.rule_id == id))
    db.session.commit()

    # new = db.execute('SELECT * FROM rules WHERE rule_id = ?', (id,)).fetchone()
    new = db.session.execute(db.select(Rules).where(Rules.rule_id == id)).scalar_one()

    insert_log("Admin: " + g.user.user_name, None, None, f'Update point policy - Rule Name: {new.rule_name} | Value: {old} -> {new.value}')
...
def update_teaching_point_balances(rule_id):
    ...
    rule = db.session.execute(db.select(Rules).where(Rules.rule_id == rule_id)).scalar_one()
    ...
    if rule_name.startswith('Role-'):
        ...
        stmt = db.select(FacultyStatus.user_id).\
            where(FacultyStatus.active_status.is_(True), FacultyStatus.role == role)
        faculty = db.session.execute(stmt).scalars()

        for user in faculty:
            db.execute('UPDATE faculty_point_info SET credit_due = ? WHERE user_id = ? AND year = ?', (rule.value, user, start_year))
        
        db.commit()
        
        for user in faculty:
            update_yearly_ending_balance(user, start_year)
        ...
        stmt = db.select(ScheduledTeaching).where(
                ((ScheduledTeaching.year == start_year) & (ScheduledTeaching.quarter == 1)) | 
                ((ScheduledTeaching.year == end_year) & ((ScheduledTeaching.quarter == 2) | (ScheduledTeaching.quarter == 3))))
        ...
        for offering in offerings:
            ...
            if value != offering.teaching_point_val:
                ...
                stmt = db.update(ScheduledTeaching).\
                    ...
                    where(ScheduledTeaching.user_id == offering.user_id,
                        ScheduledTeaching.year == offering.year,
                        ScheduledTeaching.quarter == offering.quarter,
                        ScheduledTeaching.course_title_id == offering.course_title_id,
        ...
        faculty = db.session.execute(db.select(FacultyStatus.user_id).where(FacultyStatus.active_status.is_(True))).scalars()
